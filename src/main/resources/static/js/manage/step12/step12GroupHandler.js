class Step12GroupHandler {
    ...
    tagNames = ['titles', 'carNos'];
    groupReportKey = `
                        <th>No</th>
                        <th class="groupCarNo">기사</th>
    ...
    async groupReportsRetrieval() {
        ...
        const responseData = await this.requestHandler.post('/manage/step12' + '/fetch' + '/groupOrderReportList'
    ...
    async recommendKeywordRetrieval() {
        const responseData = await new RequestHandler()
            .get('/manage/step12' + '/fetch' + '/recommendKeywordList');
    ...
    async modifyPaymentChkInBulk(type) {
        ...
        const responseData = await this.requestHandler
            .put('/manage/step12' + '/fetch' + '/paymentInBulk', this.jsonHandler.convertObjectToJson(requestObj));
